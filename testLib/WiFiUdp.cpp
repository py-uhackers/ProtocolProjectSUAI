...
int WiFiUDP::endPacket() {
  ...
  for (int retry = 0; retry <= maxRetries; ++retry) {
    sent = sendto(udp_server, tx_buffer, tx_buffer_len, 0, (struct sockaddr*) &recipient, sizeof(recipient));
    if (sent >= 0) {
      log_e("Sent %d bytes to %s:%d", sent, remote_ip.toString().c_str(), remote_port);
      return 1;
    } else {
      log_e("Could not send data: %d, retrying (%d/%d)", errno, retry + 1, maxRetries + 1);
    }
  }

  log_e("Reached maximum retries, could not send data");
  return 0;
}
...
int WiFiUDP::parsePacket(){
  ...
  if (len > 0) {
    log_e("Received %d bytes from %s:%d", len, remote_ip.toString().c_str(), remote_port);
